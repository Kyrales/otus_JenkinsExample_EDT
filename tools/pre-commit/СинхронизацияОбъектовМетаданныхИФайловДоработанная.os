...
Функция ИмяСценария() Экспорт
	...
	Возврат "СинхронизацияОбъектовМетаданныхИФайловДоработанная";
...
Функция ОбработатьФайл(АнализируемыйФайл, КаталогИсходныхФайлов, ДополнительныеПараметры) Экспорт
	...
	Если АнализируемыйФайл.Существует() И ЭтоФайлОписанияКонфигурации Тогда
		Лог.Информация("Локальная обработка файла '%1' по сценарию '%2'", АнализируемыйФайл.ПолноеИмя, ИмяСценария());
...
Функция ПолучитьОбъектыМетаданных(ИмяФайла, ЭтоПВХ = Ложь)
	...
	Для Каждого ОбъектМетаданных Из ДочерниеЭлементы.Совпадения Цикл
		...
		Если ТипМетаданных = "EnumRef" Тогда
			ТипМетаданных = "Enum";
		ИначеЕсли ТипМетаданных = "CatalogRef" Тогда
			ТипМетаданных = "Catalog";
		ИначеЕсли ТипМетаданных = "DocumentRef" Тогда
			ТипМетаданных = "Document";
		ИначеЕсли ТипМетаданных = "ChartOfAccountsRef" Тогда
			ТипМетаданных = "ChartOfAccounts";
		ИначеЕсли ЭтоПВХ Тогда
			// Если для ПВХ не определился тип, то не добавляем этот объект.
			Продолжить;
		Иначе
			// Продолжаем обработку с выбранным выше типом метаданных.
		КонецЕсли;
...
Функция СинхронизироватьМетаданныеИФайлыПВХ(Знач ИмяФайла, Знач КаталогИсходныхФайлов)
	...
	МассивОшибокПоТипуМетаданных = Новый Массив;
	Для Каждого СтрокаТипа Из ТипыМетаданных Цикл
		КаталогТипа = КаталогиТиповМетаданных.Получить(СтрокаТипа.ТипМетаданных);
		Если КаталогТипа = Неопределено Тогда
			МассивОшибокПоТипуМетаданных.Добавить(СтрШаблон("Не найден каталог типа метаданных %1", СтрокаТипа.ТипМетаданных));
			Продолжить;
		КонецЕсли;

		МассивОбъектов = МассивОбъектовМетаданных(ОбъектыМетаданных, СтрокаТипа.ТипМетаданных);
		ФайлыОбъектов = НайтиФайлыОбъектов(КаталогТипа, МассивОбъектов, МассивОшибокПоТипуМетаданных);
		МассивОшибокПоТипуМетаданных.Очистить(); 
		...
		Для Каждого ОбъектМетаданных Из МассивОбъектов Цикл
			// В массиве остались объекты, по которым не найдены файл описания и каталог объекта
			Ошибка = СтрШаблон("Отсутствуют файлы объектов конфигурации для %1.%2", СтрокаТипа.ТипМетаданных, ОбъектМетаданных);
			МассивОшибокПоТипуМетаданных.Добавить(Ошибка);
		КонецЦикла;

		Для каждого СтрокаОшибки Из МассивОшибокПоТипуМетаданных Цикл
			МассивОшибок.Добавить(Ошибка);	
		КонецЦикла;
		МассивОшибокПоТипуМетаданных.Очистить();
